...
//	aetherctl events [-n N] [-offline]      — list recent events
...
//	aetherctl patterns                      — detected patterns with confidence
//	aetherctl suggestions                   — suggestion history with status
//	aetherctl summary                       — trigger an immediate analysis cycle
//	aetherctl level                         — show current notification level
//	aetherctl level N                       — set notification level (0-4)
//	aetherctl feedback <id> accept|dismiss  — respond to a suggestion
package main
...
// Usage: aetherctl feedback <id> accept|dismiss
func cmdFeedback(socketPath string, args []string) error {
	if len(args) != 2 {
		return fmt.Errorf("usage: aetherctl feedback <id> accept|dismiss")
	}
...
func cmdConfig(socketPath string) error {
	...
	order := []string{
		"db_path", "socket_path",
		"cactus_url", "cactus_model", "cactus_route",
