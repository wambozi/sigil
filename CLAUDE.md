...
|-----------|---------|---------|
| `Source` | collector | Event emission (File, Process, Git, Terminal, Hyprland, Clipboard) |
...
1. **Collector** fans in events from 6 sources via `Source` interface, persists to store, broadcasts on channel
...
`file`, `process`, `hyprland`, `git`, `terminal`, `ai`, `clipboard` — defined in `internal/event/event.go`.
